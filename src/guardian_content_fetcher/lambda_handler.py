...
def handler(event: Dict[str, Any], context: object) -> Dict[str, Any]:
    ...
    This function is triggered by an event (e.g., from EventBridge, API Gateway, or manual invocation).
    It fetches articles from the Guardian API based on parameters in the event
    ...
    Args:
        event (Dict[str, Any]): The event data passed to the Lambda function.
                               ...
                               - 'max_articles' (int, optional): Number of articles (1-50).
    ...
    try:
        ...
        if not guardian_api_key or not kinesis_stream_name:
            raise EnvironmentError(
                "Missing required environment variables: GUARDIAN_API_KEY and KINESIS_STREAM_NAME must be set."
            )
        
        # AWS credentials (access key, secret key) are typically handled by the Lambda execution role.
        # We don't need to load them manually here.

        # --- 3. Execute the Fetch and Publish Logic ---
        logger.info(f"Initializing fetcher for Kinesis stream '{kinesis_stream_name}' in region '{aws_region}'")

...
class GuardianAPIClient:
    ...
    def _make_request(self, endpoint: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get('response', {}).get('status') != 'ok':
                error_message = data.get('response', {}).get('message', 'Unknown API error')
                raise GuardianAPIError(f"Guardian API error: {error_message}")
                
            logger.debug(f"API request successful, received {len(data.get('response', {}).get('results', []))} results")
            return data
    ...
    def search_content(
        ...
        Args:
            ...
            page_size (int): Maximum number of articles to return (default: 10)
        ...
        params = {
            ...
            'show-fields': 'webPublicationDate,webTitle,webUrl,bodyText',  # Include content preview
        }

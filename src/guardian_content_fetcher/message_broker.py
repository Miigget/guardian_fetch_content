...
class KinesisPublisher(MessageBrokerPublisher):
    ...
    def __init__(
        ...
        try:
            ...
            raise MessageBrokerError(
                "AWS credentials not found. Please configure credentials or provide them explicitly."
            )
    ...
    def _verify_stream_exists(self) -> None:
        ...
        try:
            ...
            if stream_status not in ['ACTIVE', 'UPDATING']:
                raise MessageBrokerError(
                    f"Kinesis stream '{self.stream_name}' is not active. Status: {stream_status}"
                )
                
            logger.info(f"Successfully verified Kinesis stream '{self.stream_name}' is accessible")
            
        except ClientError as e:
            error_code = e.response.get('Error', {}).get('Code', 'Unknown')
            if error_code == 'ResourceNotFoundException':
                raise MessageBrokerError(f"Kinesis stream '{self.stream_name}' does not exist")
            else:
    ...
    def publish_message(self, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(f"Message published successfully. Sequence number: {response['SequenceNumber']}")
            return True
    ...
    def publish_batch(self, messages: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            for i in range(0, len(messages), batch_size):
                ...
                if failed_count > 0:
                    logger.warning(f"Batch had {failed_count} failed records out of {len(records)}")
                else:
                    logger.debug(f"Batch of {len(records)} records published successfully")
            
            logger.info(f"Published {successful_count} out of {len(messages)} messages successfully")
...
class MockPublisher(MessageBrokerPublisher):
    ...
    def publish_message(self, message: Dict[str, Any]) -> bool:
        ...
        logger.debug(f"Mock published message: {message.get('webTitle', 'Unknown title')}")
        return True

...
class GuardianContentFetcher:
    """
    Main orchestrator class for fetching Guardian articles and publishing to
    message brokers.
    ...
    def __init__(
        self, api_client: GuardianAPIClient, message_broker: MessageBrokerPublisher
    ):
        """
        ...
        Args:
            ...
            message_broker (MessageBrokerPublisher): Configured message broker
                publisher
    ...
    def fetch_and_publish(
        self, search_term: str, date_from: Optional[str] = None, max_articles: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Starting article fetch and publish process for term: '{search_term}'"
            )
            ...
            if published_count == len(articles):
                ...
                logger.warning(
                    f"Published {published_count} out of " f"{len(articles)} articles"
                )
                error_msg = (
                    f"Only {published_count} out of {len(articles)} "
    ...
    def _validate_inputs(
        self, search_term: str, date_from: Optional[str], max_articles: int
    ) -> None:
        """
    ...
    def _fetch_articles(
        ...
        try:
            logger.debug(
                f"Fetching articles with term: '{search_term}', date_from: {date_from}"
            )
            articles = self.api_client.search_content(
                search_term=search_term, date_from=date_from, page_size=max_articles
            )

            logger.info(
                f"Successfully fetched {len(articles)} articles from Guardian API"
            )
            return articles
    ...
    def _publish_articles(self, articles: List[Dict[str, Any]]) -> int:
        ...
        try:
            ...
            if published_count == len(articles):
                logger.info(
                    f"Successfully published all {published_count} articles in batch"
                )
                return published_count
            elif published_count > 0:
                logger.warning(
                    f"Batch publish partially successful: "
                ...
                logger.warning(
                    "Batch publish failed, falling back to individual publishing"
                )
    ...
    def _publish_articles_individually(self, articles: List[Dict[str, Any]]) -> int:
        ...
        for i, article in enumerate(articles, 1):
            try:
                ...
                if success:
                    published_count += 1
                    title = article.get("webTitle", "Unknown")
                    logger.debug(f"Published article {i}/{len(articles)}: {title}")
                else:
                    title = article.get("webTitle", "Unknown")
                    logger.warning(
                        f"Failed to publish article {i}/{len(articles)}: {title}"
                    )
                ...
                logger.error(
                    f"Unexpected error publishing article {i}/{len(articles)}: {e}"
                )
                continue

        logger.info(
            f"Individual publishing completed: "
            f"{published_count}/{len(articles)} articles published"
        )
        return published_count
...
class GuardianContentFetcherFactory:
    """
    Factory class for creating GuardianContentFetcher instances with
    ...
    def create_with_kinesis(
        ...
        try:
            ...
            logger.info(
                f"Created GuardianContentFetcher with Kinesis stream: "
                f"{kinesis_stream_name}"
            )
            return fetcher

        except Exception as e:
            raise GuardianContentFetcherError(
                f"Failed to create fetcher with Kinesis: {e}"
            )

    @staticmethod
    def create_with_mock(guardian_api_key: str) -> GuardianContentFetcher:
        """
        Create a GuardianContentFetcher configured with mock message broker
        for testing.
        ...
        try:
            ...
            raise GuardianContentFetcherError(
                f"Failed to create fetcher with mock: {e}"
            )

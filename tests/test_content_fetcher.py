...
class TestGuardianContentFetcher:
    ...
    def test_fetch_and_publish_success(
        self, content_fetcher, sample_formatted_articles
    ):
        ...
        content_fetcher.api_client.search_content = Mock(
            return_value=sample_formatted_articles
        )
        ...
        content_fetcher.api_client.search_content.assert_called_once_with(
            search_term="machine learning", date_from=None, page_size=10
        )
    ...
    def test_fetch_and_publish_with_date_filter(
        self, content_fetcher, sample_formatted_articles
    ):
        ...
        result = content_fetcher.fetch_and_publish(
            "machine learning", date_from="2023-01-01", max_articles=5
        )
        ...
        content_fetcher.api_client.search_content.assert_called_once_with(
            search_term="machine learning", date_from="2023-01-01", page_size=5
        )
    ...
    def test_fetch_and_publish_api_error(self, content_fetcher):
        ...
        with pytest.raises(
            GuardianContentFetcherError, match="Service error during fetch and publish"
        ):
            content_fetcher.fetch_and_publish("test")

    def test_fetch_and_publish_message_broker_error(
        self, content_fetcher, sample_formatted_articles
    ):
    ...
    def test_fetch_and_publish_partial_publish_success(
        self, content_fetcher, sample_formatted_articles
    ):
    ...
    def test_fetch_and_publish_complete_publish_failure(
        self, content_fetcher, sample_formatted_articles
    ):
    ...
    def test_fetch_and_publish_fallback_to_individual_publishing(
        self, content_fetcher, sample_formatted_articles
    ):
    ...
    def test_unexpected_error_handling(self, content_fetcher):
        ...
        with pytest.raises(
            GuardianContentFetcherError,
            match="Unexpected error during fetch and publish",
        ):
            content_fetcher.fetch_and_publish("test")
...
class TestGuardianContentFetcherFactory:
    ...
    def test_create_with_kinesis(
        self, mock_api_client_class, mock_kinesis_publisher_class
    ):
    ...
    @patch("guardian_content_fetcher.message_broker.KinesisPublisher")
    @patch("guardian_content_fetcher.api_client.GuardianAPIClient")
    def test_create_with_kinesis_default_params(
        self, mock_api_client_class, mock_kinesis_publisher_class
    ):
    ...
    def test_create_with_kinesis_error(self, mock_kinesis_publisher_class):
        ...
        with pytest.raises(
            GuardianContentFetcherError, match="Failed to create fetcher with Kinesis"
        ):
    ...
    def test_create_with_mock_error(self, mock_api_client_class):
        ...
        with pytest.raises(
            GuardianContentFetcherError, match="Failed to create fetcher with mock"
        ):

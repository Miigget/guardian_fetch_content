...
class TestGuardianAPIClient:
    ...
    def test_rate_limiting(self, guardian_api_client):
        ...
        assert (
            time_diff >= guardian_api_client.rate_limit_delay * 0.9
        )  # Allow for timing variations

    def test_search_content_success(
        self, guardian_api_client, mock_requests_get, sample_guardian_api_response
    ):
    ...
    def test_search_content_with_date_filter(
        ...
        articles = guardian_api_client.search_content(
            "machine learning", date_from="2023-01-01"
        )
    ...
    def test_search_content_with_custom_page_size(
        self, guardian_api_client, mock_requests_get, sample_guardian_api_response
    ):
        ...
        articles = guardian_api_client.search_content("machine learning", page_size=5)
    ...
    def test_search_content_api_error_response(
        self, guardian_api_client, mock_requests_get, error_response
    ):
    ...
    def test_search_content_http_error(self, guardian_api_client, mock_requests_get):
        ...
        with pytest.raises(
            GuardianAPIError, match="Failed to make request to Guardian API"
        ):
            guardian_api_client.search_content("test")
    ...
    def test_search_content_invalid_json(self, guardian_api_client, mock_requests_get):
        ...
        with pytest.raises(
            GuardianAPIError, match="Invalid JSON response from Guardian API"
        ):
            guardian_api_client.search_content("test")

    def test_search_content_no_results(
        self, guardian_api_client, mock_requests_get, empty_response
    ):
    ...
    def test_search_content_with_long_body_text(
        self, guardian_api_client, mock_requests_get
    ):
    ...
    def test_search_content_strips_whitespace(
        self, guardian_api_client, mock_requests_get, sample_guardian_api_response
    ):
    ...
    def test_search_content_with_special_characters(
        self, guardian_api_client, mock_requests_get, sample_guardian_api_response
    ):
